...
const mySaveObject = saveCollection.find(
  (saveFile) => saveFile.name === machineID.toLowerCase()
);
const mySaveFolder = mySaveObject.folder;
...
function addDir(changedPath) {
  ...
  const pathToTargetOtherPlayer = path.join(
    pathToRepo,
    saveFolders,
    otherSaveFolder,
    filename
...
async function syncToLocal() {
  ...
  fs.cpSync(path.resolve(localPath), localappdata, {
    recursive: true,
    force: true,
  });
  console.debug("end 2");
